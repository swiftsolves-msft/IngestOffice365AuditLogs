...
# ===================================================================
# Map a raw O365 event object to a compact schema for Data Lake ingestion.
# ===================================================================
function Convert-O365Record {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [psobject]$AuditEvent,
    ...
    $tg = if ($AuditEvent.CreationTime) {
        ConvertTo-IsoUtcString $AuditEvent.CreationTime
    } else { $currentUTCtime.ToString("o") }

    return [ordered]@{
        TimeGenerated    = $tg
        RawData          = ($AuditEvent | ConvertTo-Json -Depth 10 -Compress)
        Operation_s      = if ($null -ne $AuditEvent.Operation) { [string]$AuditEvent.Operation } else { $null }
        UserId_s         = if ($null -ne $AuditEvent.UserId) { [string]$AuditEvent.UserId } else { $null }
        RecordType_d     = if ($null -ne $AuditEvent.RecordType) { [double]$AuditEvent.RecordType } else { $null }
        Workload_s       = if ($null -ne $AuditEvent.Workload) { [string]$AuditEvent.Workload } else { $null }
        ResultStatus_s   = if ($null -ne $AuditEvent.ResultStatus) { [string]$AuditEvent.ResultStatus } else { $null }
        OrganizationId_g = if ($null -ne $AuditEvent.OrganizationId) { [string]$AuditEvent.OrganizationId } else { $null }
        Id_g             = if ($null -ne $AuditEvent.Id) { [string]$AuditEvent.Id } else { $null }
        ClientIP_s       = if ($null -ne $AuditEvent.ClientIP) { [string]$AuditEvent.ClientIP } else { $null }
        CreationTime_t   = ConvertTo-IsoUtcString $AuditEvent.CreationTime
        ContentType_s    = $ContentType
    }
}
    ...
    foreach($contentType in $contentTypes){
        ...
        do {
            ...
            foreach($obj in $contentResult){
                ...
                $blobData = @()
                foreach($o365Event in $data){
                    $matchesFilter = $false
                    if($Office365RecordTypes -eq "0"){
                        if(($o365Event.Source) -ne "Cloud App Security"){
                            $matchesFilter = $true
                        }
                    }
                    else {
                        $types = ($Office365RecordTypes).split(",")
                        if(($o365Event.RecordType) -in $types){
                            $matchesFilter = $true
                        }
                    }

                    if ($matchesFilter) {
                        $blobData += Convert-O365Record -AuditEvent $o365Event -ContentType $contentType
                    }
                }
